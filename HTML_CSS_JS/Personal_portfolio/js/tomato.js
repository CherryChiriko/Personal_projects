  ...
  alertTime = document.getElementById("alert");
  const MINUTES = 25; const SECONDS = 0;
  ...
  function timer(){
    if(minutes==0 && seconds==0){ clearInterval(display); start.disabled = true; stop.disabled = true; return;}
  ...
  function alarm(){ alertTime.style.visibility='visible'; beep();}
  function beep() {
  ...
  alertTime.style.visibility='hidden';

  let minutes = MINUTES;  let seconds = SECONDS;
  display_number (minutes, seconds);

  closebtn.onclick= () => { alertTime.style.visibility='hidden';}
  ...
  start.onclick   = () => {
    if (!pause){
      ...
      myTimeout = setTimeout(alarm, 1000*(seconds)+60000*minutes);
