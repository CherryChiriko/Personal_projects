...
def parent_finder(h, n):
    if n == 2**h - 1: return -1
    global_offset = 0                                           # treat the right branches as left ones + an offset (shifting the subtree)
    while h :
        max_n = 2**h - 1; left_max = (max_n - 1)//2             # root is max_n and left_max is the root of the left branch
        if n == max_n : return 2**(h+1) - 1 + global_offset
        elif n == left_max*2 : return n + global_offset + 1     # if the last step is a right branch, simply return the number + offset + 1
        elif n > left_max :                                     # right branch
            global_offset += left_max                           # the right branch is calculated in function of the left branch, with an offset
            n -= left_max
        h -= 1

def solution(h, q) :
    return [parent_finder(h,n) for n in q]

# print(solution(5, [5]))
print(solution(5, [4, 21, 5]))
# print(solution(3, [7, 3, 5, 1, 4, 2]))
